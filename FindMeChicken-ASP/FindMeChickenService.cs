...
namespace FindMeChicken_ASP
{
    ...
    public class GetChickenMenuService : Service
    {
        ...
        {
            if (req.SourceName == null || req.Id == null || !FindMeChickenService.SOURCES.ContainsKey(req.SourceName)) return null;
    ...
    public class FindMeChickenService : Service
    {
        ...
        {
            if (req.Lat == 0 && req.Long == 0) return new ChickenSearchRequestResponse() { Result = null };
            ...
            timer.Stop();
            Debug.Print(string.Format("Took {0} to fetch places", timer.Elapsed.ToString()));
